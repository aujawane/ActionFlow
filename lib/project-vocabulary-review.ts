...
// path back into "suggested" (re-suggesting is upsertVocabularySuggestions's job, not this
// module's), and this is the review step, not a general-purpose field editor.
 ...
 *
 * Lives in lib/, not in the route.ts that calls it: a Next.js route module may only export
 * supported route handlers/config (GET/POST/PATCH/.../runtime/dynamic/...) -- any other export,
 * including a plain helper function meant only for tests, fails `next build`'s route-shape
 * validation ("... is not a valid Route export field"). Factoring the real logic out here, with an
 * injectable dependencies parameter, mirrors the existing createResponse-override pattern used for
 * model calls elsewhere in this repo (e.g. lib/execution-intelligence/work-item-model.ts's
 * `createResponse` parameter) rather than inventing a new testing convention. Production always
 * uses `defaultDependencies`; the authorization order and every response shape/status code are
 * unchanged from a plain inline implementation -- this only adds a seam, not new behavior.
 */
