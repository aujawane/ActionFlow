-- Forward-only. Fixes the "preserved rows become invisible" failure mode identified by the
-- read-only Production extraction-integrity audit (see the commit introducing this migration).
--
-- Root cause: replace_meeting_execution_graph() correctly protects preserve_on_reanalysis = true
-- rows from deletion when a later analysis run doesn't re-match them, but it never refreshed
-- their metadata->>'analysis_generation' / extraction_metadata->>'analysis_generation' stamp
-- either. Every read-time "is this row current" check (lib/execution-generation.ts's
-- isCommitmentCurrentGeneration / isTaskCurrentGeneration, used by Meeting Detail, Project
-- execution aggregation, and Project Brain) requires an exact generation match, so a preserved
-- row -- and the human edit it protects -- silently vanished from every one of those views even
-- though it was never deleted and remained fully valid.
--
-- Fix: for any existing task/commitment that is preserve_on_reanalysis = true and was not
-- matched into this run's persistence payload, carry its generation stamp forward to the
-- generation that just successfully persisted. Only the analysis_generation field changes --
-- every other column (including anything already protected by manual_override_fields) is
-- untouched, so no human-edited value is altered. This keeps the existing generation-equality
-- read-time checks unchanged and correct: after this, a preserved row's stamp always matches
-- last_persisted_execution_generation unless a genuinely newer, matched replacement exists for
-- it, so no special-casing is needed in any TypeScript consumer and no duplicate "current" row
-- can appear for the same preserved item.
...
begin
  ...
  -- New: carry forward the generation stamp on preserved rows this run didn't match, so they
  -- stay visible to every read-time "is this row current" check without altering any
  -- human-edited value (only the metadata->>'analysis_generation' field changes; manual_override_
  -- fields-protected columns and all other data are untouched). Non-preserved unmatched rows are
  -- intentionally left alone here -- they're either deleted below or, if referenced by artifacts/
  -- comments, correctly remain stale (that retention path is unrelated to this fix).
  update public.meeting_tasks task
  set extraction_metadata = extraction_metadata || jsonb_build_object('analysis_generation', p_generation)
  where task.id = any(old_task_ids)
    and not (task.id = any(matched_task_ids))
    and task.preserve_on_reanalysis = true;

  update public.meeting_commitments commitment
  set metadata = metadata || jsonb_build_object('analysis_generation', p_generation)
  where commitment.id = any(old_commitment_ids)
    and not (commitment.id = any(matched_commitment_ids))
    and commitment.preserve_on_reanalysis = true;
...
comment on function public.replace_meeting_execution_graph(uuid, bigint, jsonb, jsonb) is
  'Atomically replace a meeting execution graph for one analysis generation, preserving protected rows and execution_classification. Preserved rows not matched by this run have their analysis_generation stamp carried forward so they remain visible to read-time generation-currency checks.';
