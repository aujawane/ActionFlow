-- Additive commitment-first classification fields.
-- Backward compatible: existing rows default to 'committed'.
...
begin
  if not exists (
    ...
    where t.typname = 'execution_classification'
...
add column if not exists execution_classification public.execution_classification
...
create index if not exists meeting_commitments_meeting_classification_idx
on public.meeting_commitments (meeting_id, execution_classification);

create index if not exists meeting_tasks_meeting_classification_idx
on public.meeting_tasks (meeting_id, execution_classification);

create index if not exists meeting_tasks_commitment_status_idx
